dotnetify_blazor = {
  ...
  addEventListener: function (event, elem, callbackHelper) {
    ...
    if (!(elem && typeof elem.addEventListener === 'function')) throw "Cannot listen to event '" + event + "': invalid VMContext @ref.";
    ...
    if (elem.getAttribute('vm') && event === 'onStateChange') {
      ...
      if (parent) {
        ...
        const restore = () => {
          ...
          if (placeholder) {
            while (tempParent.childNodes.length > 0) placeholder.parentElement.appendChild(tempParent.childNodes[0]);
        ...
        while (elemParent.childNodes.length > 0) tempParent.appendChild(elemParent.childNodes[0]);
    ...
    const callback = e => {
      callbackHelper.invokeMethodAsync('Callback', JSON.stringify(e.detail,
        // Don't stringify DOM elements.
        (_, value) => value instanceof HTMLElement ? value.id : value
      ));
    };
  ...
  configure: function (config) {
    ...
    if (config.hubServerUrl) dotnetify.hubServerUrl = config.hubServerUrl;
    if (config.webSocketServerUrl) dotnetify.hub = dotnetify.createWebSocketHub(config.webSocketServerUrl);
  },
  dispatch: function (elem, state) {
    if (!(elem && elem.vm)) throw 'Invalid VMContext @ref.';
